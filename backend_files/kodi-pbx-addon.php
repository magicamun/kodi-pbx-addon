...
$__mysqlhost__		= "localhost";
$__mysqluser__		= "cdr";
$__mysqlpasswd__	= "cdr";
$__mysqldbname__	= "asteriskcdrdb";
$__cdr_max__		= 500;
...
elseif (isset($_GET["cdr"]) || isset($_GET["vm"])) {
    ...
    if (isset($_GET["cdr"])) {
	if ($__usecdrdb__ == "yes") {
            $db = mysqli_connect($__mysqlhost__, $__mysqluser__, $__mysqlpasswd__, $__mysqldbname__);
            if($db) {
	        $cdr = mysqli_query($db, "select * from cdr_kodi order by start desc");
		if (!$cdr) {
                    $node = $xmldoc->createElement("error");
                    $element = $xmldoc->createElement("mysql");
                    $element->appendChild($xmldoc->createTextNode(utf8_encode(mysql_error($db))));
                    $node->appendChild($element);
		   
                    $xmlroot->appendChild($node);
		}
		$i = 0;
	        while ($row = mysqli_fetch_object($cdr) and $i++ < $__cdr_max__) {
                    $node = $xmldoc->createElement("cdr");
		    foreach ($row as $key=>$data) {
                        $element = $xmldoc->createElement($key);
                        $element->appendChild($xmldoc->createTextNode(utf8_encode($data)));
                        $node->appendChild($element);
		    }
                    $xmlroot->appendChild($node);
                }
            ...
            for ($i=0; ($i < count($cdr) and $i < $__cdr_max__); $i++) {
                $node = $xmldoc->createElement("cdr");
                for ($c=0; $c < count($cdr[$i]); $c++) {
		    $key = $cdr_fields[$c];
		    $data = $cdr[$i][$c];
		    # convert dat-time strings to utc-seconds
		    if ($key == "start" || $key == "answer" || $key == "end") {
			$data = get_utc($data);
		    }
                    $element = $xmldoc->createElement($key);
                    $element->appendChild($xmldoc->createTextNode(utf8_encode($data)));
    ...
    $html_content .= "</td></tr>\n";
    if ($__usecdrdb__ == "yes") {
        $html_content .= "<tr><td>Use Mysql-database for cdr<br/>\$__usecdrdb__ is set to</td><td>$__usecdrdb__</td></tr>\n";
        $html_content .= "<tr><td>Able to connect to cdr-database<br/Is the connection configured properly?)</td><td>";
        $db = mysqli_connect($__mysqlhost__, $__mysqluser__, $__mysqlpasswd__, $__mysqldbname__);
        if(!$db) {
            $html_content .= "ERROR";
            $found_err = true;
        ...
        $html_content .= "<tr><td>Use plain file for cdr<br/>\$__usecdrdb__ is set to</td><td>$__usecdrdb__</td></tr>\n";
    ...
    if (is_readable($ami_path)) {
        if ($handle = opendir($ami_path)) {
            ...
            foreach ($ami as $arr_tmp) {
                if (array_key_exists("kodi",$arr_tmp)) {
                    $ami_found = true;
                }
            }
        }
    ...
    if ($found_err) {
        ...
        $html_content .= "<p>Seems everything is ok on the Back-end (PBX Server) side. 
                            <br/>Please continue the setup on the Front-end (Kodi) side.</p>\n";
    }
...
function get_utc($datetime) {
    return (string)strtotime($datetime);
}
?>
