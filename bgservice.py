    ...
    def newcall_actions(self,event):
        ...
        if (xbmc_oncall_notification):
            ...
            if (xbmc_caller_picture_enabled):
                xbmc_caller_picture = xbmc_caller_picture_path + self.event_callerid_num + ".png"
                if (self.DEBUG):
                    log(">> Caller's picture: " + xbmc_caller_picture)
                if (xbmcvfs.exists(xbmc_caller_picture)):
                    xbmc.executebuiltin("XBMC.Notification("+ __language__(30050) +","+ xbmc_notification +","+ str(xbmc_oncall_notification_timeout*1000) +","+ xbmc_caller_picture +")")
                else:
                    log(">> No Picture found - falling back to std-icon!")
...
try:
    ...
    if (xbmc_bgservice == "true"):
        ...
        xbmc_img = xbmc.translatePath(os.path.join(RESOURCE_PATH,'media','kodi-pbx-addon.png'))
        log(">> Notification: " + xbmc_notification)
        log(">> Notification: LT1")
        log(">> Notification: LT1a " + __language__(30052))
        ...
        log(">> Notification: LT2")
        grab = get_incoming_call()
        log(">> Notification: LT3")
